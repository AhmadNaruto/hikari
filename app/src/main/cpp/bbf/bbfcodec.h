...
class BBFBuilder
{
    public:
        ...
        // Asset and page management additions
        int64_t addAsset(const char* fPath, uint32_t aFlags = 0);
        int64_t addAssetWithPath(const char* fPath, const char* logicalPath, uint32_t aFlags = 0);
        int64_t addAssetFromMemory(const uint8_t* buffer, uint64_t size, const char* fExtOrName, uint32_t aFlags = 0);
        int64_t addAssetFromMemoryWithPath(const uint8_t* buffer, uint64_t size, const char* fExtOrName, const char* logicalPath, uint32_t aFlags = 0);
        bool addPageByAssetIndex(uint64_t assetIndex, uint32_t pFlags = 0);
        bool addPageWithPath(const char* fPath, const char* logicalPath, uint32_t pFlags = 0, uint32_t aFlags = 0);
...
class BBFReader
{
    public:
        ...
        // Asset path and lookup utility
        int64_t findAssetByPath(const char* path);
        const char* getAssetPath(int assetIndex);
