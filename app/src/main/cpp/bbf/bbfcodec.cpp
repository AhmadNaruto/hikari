...
static const uint32_t PACK4_TXT_  = PACK4('t', 'x', 't', ' ');
static const uint32_t PACK4_HTML_ = PACK4('h', 't', 'm', 'l');
static const uint32_t PACK4_HTM_  = PACK4('h', 't', 'm', ' ');
static const uint32_t PACK4_XHTML_= PACK4('x', 'h', 't', 'm');
static const uint32_t PACK4_CSS_  = PACK4('c', 's', 's', ' ');
static const uint32_t PACK4_JSON_ = PACK4('j', 's', 'o', 'n');
static const uint32_t PACK4_EPUB_ = PACK4('e', 'p', 'u', 'b');
static const uint32_t PACK4_TTF_  = PACK4('t', 't', 'f', ' ');
static const uint32_t PACK4_OTF_  = PACK4('o', 't', 'f', ' ');
static const uint32_t PACK4_WOFF_ = PACK4('w', 'o', 'f', 'f');
static const uint32_t PACK4_PDF_  = PACK4('p', 'd', 'f', ' ');
static const uint32_t PACK4_MD_   = PACK4('m', 'd', ' ', ' ');
static const uint32_t PACK4_MARK_ = PACK4('m', 'a', 'r', 'k');
static const uint32_t PACK4_SVG_  = PACK4('s', 'v', 'g', ' ');
static const uint32_t PACK4_JS_   = PACK4('j', 's', ' ', ' ');
...
uint8_t BBFBuilder::detectType(const char* fPath)
{
    ...
    switch (PACKED)
    {
        ...
        case PACK4_TXT_: return (uint8_t)BBF::BBFMediaType::TXT;
        case PACK4_HTML_: return (uint8_t)BBF::BBFMediaType::HTML;
        case PACK4_HTM_: return (uint8_t)BBF::BBFMediaType::HTML;
        case PACK4_XHTML_: return (uint8_t)BBF::BBFMediaType::HTML;
        case PACK4_CSS_: return (uint8_t)BBF::BBFMediaType::CSS;
        case PACK4_JSON_: return (uint8_t)BBF::BBFMediaType::JSON;
        case PACK4_EPUB_: return (uint8_t)BBF::BBFMediaType::EPUB;
        case PACK4_TTF_: return (uint8_t)BBF::BBFMediaType::TTF;
        case PACK4_OTF_: return (uint8_t)BBF::BBFMediaType::OTF;
        case PACK4_WOFF_: {
            if (fExt[4] == '2') {
                return (uint8_t)BBF::BBFMediaType::WOFF2;
            }
            return (uint8_t)BBF::BBFMediaType::WOFF;
        }
        case PACK4_PDF_: return (uint8_t)BBF::BBFMediaType::PDF;
        case PACK4_MD_: return (uint8_t)BBF::BBFMediaType::MD;
        case PACK4_MARK_: return (uint8_t)BBF::BBFMediaType::MD;
        case PACK4_SVG_: return (uint8_t)BBF::BBFMediaType::SVG;
        case PACK4_JS_: return (uint8_t)BBF::BBFMediaType::JS;
...
int64_t BBFBuilder::addAsset(const char* fPath, uint32_t aFlags)
{
    uint8_t mediaType = detectType(fPath);
    ...
    int64_t newIndex = (int64_t)this->assetCount;
    this->assetCount++;

    return newIndex;
}

int64_t BBFBuilder::addAssetWithPath(const char* fPath, const char* logicalPath, uint32_t aFlags)
{
    int64_t index = addAsset(fPath, aFlags);
    if (index >= 0 && logicalPath)
    {
        char key[64];
        snprintf(key, sizeof(key), "path:%lld", (long long)index);
        addMeta(key, logicalPath);
    }
    return index;
}

int64_t BBFBuilder::addAssetFromMemory(const uint8_t* buffer, uint64_t size, const char* fExtOrName, uint32_t aFlags)
{
    if (!buffer || size == 0) return -1;

    uint8_t mediaType = detectType(fExtOrName);

    XXH128_hash_t iHash = XXH3_128bits(buffer, size);
    ...
    this->assets[this->assetCount].assetHash[0] = iHash.low64;
    this->assets[this->assetCount].assetHash[1] = iHash.high64;
    this->assets[this->assetCount].fileSize = size;
    this->assets[this->assetCount].flags = aFlags;
    this->assets[this->assetCount].type = mediaType;
...
int64_t BBFBuilder::addAssetFromMemoryWithPath(const uint8_t* buffer, uint64_t size, const char* fExtOrName, const char* logicalPath, uint32_t aFlags)
{
    int64_t index = addAssetFromMemory(buffer, size, fExtOrName, aFlags);
    if (index >= 0 && logicalPath)
    {
        char key[64];
        snprintf(key, sizeof(key), "path:%lld", (long long)index);
...
bool BBFBuilder::addPageByAssetIndex(uint64_t assetIndex, uint32_t pFlags)
{
    if (assetIndex >= this->assetCount) return false;
    ...
    this->pages[this->pageCount].assetIndex = assetIndex;
    this->pages[this->pageCount].flags = pFlags;
    this->pageCount++;
...
bool BBFBuilder::addPageWithPath(const char* fPath, const char* logicalPath, uint32_t pFlags, uint32_t aFlags)
{
    int64_t index = addAssetWithPath(fPath, logicalPath, aFlags);
    if (index >= 0)
    {
        return addPageByAssetIndex(index, pFlags);
    }
    return false;
}
